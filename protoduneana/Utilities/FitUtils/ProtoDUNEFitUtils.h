...
namespace protoana{
  namespace ProtoDUNEFitUtils{
    ...
    std::vector<TCanvas*> PlotDatasetsAndPdfs(RooWorkspace *work, TString name, TString error, TString plottodraw, std::vector<TString> binnames, std::vector<double> recobins, std::vector<TString> incidentBinNames, TString measurement="PDFit", RooAbsData* data=NULL, RooFitResult* result=NULL);
