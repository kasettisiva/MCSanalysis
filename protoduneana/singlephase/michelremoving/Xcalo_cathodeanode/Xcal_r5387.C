#define Xcal_r5387_cxx
#include "Xcal_r5387.h"
#include <TH2.h>
#include <TH3.h>
#include <TStyle.h>
#include <TCanvas.h>
...
TFile *ef = TFile::Open("$DUNE_PARDATA_DIR/SpaceChargeProtoDUNE/SCE_DataDriven_180kV_v3.root");
...
void Xcal_r5387::Loop()
{
  ...
  fChain->GetEntry(0);
  TFile my_file(Form("YZcalo_r%d.root",run));
  ...
  TFile *file = new TFile(Form("Xcalcatanode_r%d.root",run),"recreate");
...
int main(int argc, char *argv[]) {
  ...
  if (infile.substr(infile.find_last_of(".") + 1) == "root"){
    shtree->Add(Form("%s/michelremoving/Event", infile.c_str()));
  }
